...
// --- State ---
let mediaRecorder, audioChunks = [], currentAudio = null
let isPlaying = false, isRecording = false
let stream = null, silenceCheckId = null, silenceTimeoutId = null
...
document.addEventListener('click', function (event) {
  if (!modelsButton.contains(event.target) && !dropdown.contains(event.target)) {
...
document.addEventListener('click', (e) => {
    ...
    if (channelContainer) {
        const selectedButton = channelContainer.querySelector('.channel-button');
      if (selectedButton) {
...
async function handleSendText() {
  ...
  const text = value.replace(/<br\s*\/?>/gi, '').replace(/&nbsp;/g, ' ').trim()
  if (!text) return
...
async function processChatRequest(formData) {
  try {
    ...
    if (!response.ok) {
      ...
      appendMessage(SenderType.AI, 'Failed to get response from the AI.' + response.textContent,false,true)
      return
    }
    ...
    while (true) {
      ...
      const { text, foundAudioUrl, foundResolvedText } = processChunk(chunk)
...
function processChunk(chunk) {
  ...
  if (jsonStartIndex !== -1 && jsonEndIndex !== -1 && jsonEndIndex > jsonStartIndex) {
    const jsonChunk = chunk.substring(jsonStartIndex + JSON_START.length, jsonEndIndex);
    try {
      const parsedJson = JSON.parse(jsonChunk);
      if (parsedJson.resolved_text) foundResolvedText = parsedJson.resolved_text;
    } catch {}
    remainingText = chunk.slice(0, jsonStartIndex) + chunk.slice(jsonEndIndex + JSON_END.length);
  }

  const audioStartIndex = remainingText.indexOf(AUDIO_START);
  const audioEndIndex = remainingText.indexOf(AUDIO_END, audioStartIndex + AUDIO_START.length);

  if (audioStartIndex !== -1 && audioEndIndex !== -1 && audioEndIndex > audioStartIndex) {
    const audioChunk = remainingText.substring(audioStartIndex + AUDIO_START.length, audioEndIndex);
    try {
    ...
    remainingText = remainingText.slice(0, audioStartIndex) + remainingText.slice(audioEndIndex + AUDIO_END.length);
  }
  ...
  try {
    ...
    if (Array.isArray(data.history)) {
      data.history.forEach(msg => {
        const sender = msg.role.toUpperCase() === SenderType.USER ? SenderType.USER : SenderType.AI
...
function selectChannel(selectedButton) {
  const buttons = Array.from(channelList.querySelectorAll('.channel-button')).filter(
      (btn) => btn.id !== 'noHistoryButton'
  );
...
if (channelId && channelId !== currentChannelId) {
  console.log("Found channelId from URL:", channelId)
    const selectedButton = channelList.querySelector(`li[id="${channelId}"]`);
    if(selectedButton) {
...
try {
    const response = await fetch(`/api/history/${channelId}/`, { method: "DELETE" });
...
async function deleteAllHistory() {
    try {
        const response = await fetch("/api/history/delete-all", { method: "DELETE" });
        if (!response.ok) throw new Error("Failed to delete history");
        $empty(chatContainer);
        $empty(channelList);
        populateChannels();
...
searchInput.addEventListener('input', () => {
    ...
    channels.forEach(channel => {
      ...
      if (!filter || (button && button.classList.contains("new-channel-container") && button.textContent.toLowerCase().includes(filter))) {
...
// --- UI Helpers ---
function appendMessage(sender, text, streaming = false, isRed = false, audioUrl = "") {
  let bubble
  if (streaming && sender === SenderType.AI) {
    bubble = $('response-stream-bubble')
    if (!bubble) {
      bubble = $ce('div')
      bubble.id = 'response-stream-bubble'
      bubble.className = 'p-3 rounded self-start mr-auto max-w-fit break-words message-animate ai-streaming'
    ...
    bubble.style.backgroundColor = sender === SenderType.USER ? '#303030' : 'transparent'
    bubble.style.marginBottom = '0.5rem'
...
async function playResponseAudio(url) {
    ...
    currentAudio.onended = () => { isPlaying = false; toggleSendButton(false) }
    isPlaying = true
    toggleSendButton(true)
...
function showDeleteModal() {
    ...
    confirmButton.className = 'bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition';
    confirmButton.style.borderRadius = "50px"
    
    const cancelButton = $ce('button');
    cancelButton.textContent = 'Cancel';
    cancelButton.className = 'text-white px-4 py-2 rounded hover:bg-gray-700 transition';
...
function handleTouchEnd() {
  if (Math.abs(touchMoveX - touchStartX) < 50 && isSidebarOpen) closeSidebar()
  }
...
async function startRecording() {
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      if (recordingDuration < minimumRecordingTimeMs && supportedMimeType.includes('webm')) {
        appendMessage(SenderType.AI, "Recording too short, please record for at least 1 second");
        audioChunks = [];
        return;
      }
      ...
      try {
        ...
        appendMessage(SenderType.AI, "Error processing audio: " + error.message);
      }
    };
